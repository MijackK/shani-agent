...
class SkinConfig:
    ...
    def get_spinner_wings(self) -> List[Tuple[str, str]]:
        ...
        return [
            ...
            for pair in self.spinner.get("wings", [])
            if isinstance(pair, (list, tuple)) and len(pair) == 2
        ]


def _branding(
    who: str, symbol: str, goodbye: str, prompt: str = "", help_header: str = ""
) -> Dict[str, str]:
    ...
    return {
        ...
        "goodbye": goodbye,
        "response_label": f" {symbol} {who} ",
        "prompt_symbol": prompt or symbol,
...
def _wings(*glyphs) -> List[List[str]]:
    ...
    return [
        [
            f"⟪{g[0] if isinstance(g, tuple) else g}",
            f"{g[1] if isinstance(g, tuple) else g}⟫",
        ]
        for g in glyphs
    ]
...
def _build_skin_config(data: Dict[str, Any]) -> SkinConfig:
    ...
    return SkinConfig(
        ...
        light_colors=section("light_colors"),
        dark_colors=section("dark_colors"),
        spinner=merged("spinner"),
        branding=merged("branding"),
        tool_prefix=data.get("tool_prefix", default.get("tool_prefix", "┊")),
        tool_emojis=section("tool_emojis"),
...
def load_skin(name: str) -> SkinConfig:
    ...
    return _build_skin_config(
        data or _BUILTIN_SKINS.get(name) or _BUILTIN_SKINS["default"]
    )
...
def init_skin_from_config(config: dict) -> None:
    ...
    skin_name = (
        display.get("skin", "default") if isinstance(display, dict) else "default"
    )
    set_active_skin(
        skin_name.strip()
...
_STYLE_PALETTE = (
    ("prompt", "prompt", ""),
    ("input_rule", "input_rule", "#CD7F32"),
    ("title", "banner_title", "#FFD700"),
    ("text", "banner_text", "#FFF8DC"),
    ("dim", "banner_dim", "#555555"),
    ("label", "ui_label", "@title"),
    ("warn", "ui_warn", "#FF8C00"),
    ("error", "ui_error", "#FF6B6B"),
    ("status_bg", "status_bar_bg", "#1a1a2e"),
    ("status_text", "status_bar_text", "@text"),
    ("status_strong", "status_bar_strong", "@title"),
    ("status_dim", "status_bar_dim", "@dim"),
    ("ok", "ui_ok", "#8FBC8F"),
    ("status_good", "status_bar_good", "@ok"),
    ("status_warn", "status_bar_warn", "@warn"),
    ("accent", "banner_accent", "@warn"),
    ("status_bad", "status_bar_bad", "@accent"),
    ("status_critical", "status_bar_critical", "@error"),
    ("voice_bg", "voice_status_bg", "@status_bg"),
    ("menu_bg", "completion_menu_bg", "#1a1a2e"),
    ("menu_current_bg", "completion_menu_current_bg", "#333355"),
...
_STYLE_TEMPLATES = {
    ...
    "prompt": "{prompt}",
    "prompt-working": "{dim} italic",
    "hint": "{dim} italic",
    "status-bar": "bg:{status_bg} {status_text}",
    "status-bar-strong": "bg:{status_bg} {status_strong} bold",
    "status-bar-session-title": "bg:{badge_bg} {badge_fg} bold",
    "status-bar-dim": "bg:{status_bg} {status_dim}",
    "status-bar-good": "bg:{status_bg} {status_good} bold",
    "status-bar-warn": "bg:{status_bg} {status_warn} bold",
    "status-bar-bad": "bg:{status_bg} {status_bad} bold",
    ...
    "image-badge": "{label} bold",
    "completion-menu": "bg:{menu_bg} {text}",
    "completion-menu.completion": "bg:{menu_bg} {text}",
    ...
    "clarify-title": "{title} bold",
    "clarify-question": "{text} bold",
    "clarify-choice": "{dim}",
    "clarify-selected": "{title} bold",
    "clarify-active-other": "{title} italic",
    "clarify-countdown": "{input_rule}",
    "sudo-prompt": "{error} bold",
    "sudo-border": "{input_rule}",
    "sudo-title": "{error} bold",
    "sudo-text": "{text}",
    "approval-border": "{input_rule}",
    "approval-title": "{warn} bold",
    "approval-desc": "{text} bold",
    "approval-cmd": "{dim} italic",
    "approval-choice": "{dim}",
    "approval-selected": "{title} bold",
    "voice-status": "bg:{voice_bg} {label}",
    "voice-status-recording": "bg:{voice_bg} {error} bold",
}
...
def get_prompt_toolkit_style_overrides() -> Dict[str, str]:
    ...
    for name, key, fallback in _STYLE_PALETTE:
        palette[name] = skin.get_color(
            key, palette[fallback[1:]] if fallback.startswith("@") else fallback
        )
